...
(function ($) {
    ...
    var defaults = {
        ...
        delimiters: [9, 13, 42, 44], // tab, enter, *, comma
        backspace: [8],
        isInserirTR: false,
        asterisco: false,
        maxTags: 0,
    ...
    publicMethods = {
        pushTag: function (tag, ignoreEvents, externalTagId) {
            ...
            if (alreadyInList) {
                ...
                if (opts.isInserirTR && opts.asterisco) {
                    $('#dados_descricao_tr').tagsManager('pushTag', tag);
                    opts.asterisco = false;
                }
        ...
        init: function (options) {
            ...
            this.each(function () {
                ...
                $self.on('keypress', function (event) {
                    ...
                    if (privateMethods.keyInArray(event, opts.delimiterChars)) {
                        if (event.keyCode === 42) {
                            if (opts.isInserirTR) {
                                opts.asterisco = true;
                            }
                        ...
                        privateMethods.applyDelimiter.call($self, event);
                    }
                });
