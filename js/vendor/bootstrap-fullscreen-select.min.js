"use strict";
/*
 * ! Bootstrap-fullscreen-select v1.5.1
 * (http://craftpip.github.io/bootstrap-fullscreen-select/) Author: boniface
 * pereira Website: www.craftpip.com Contact: hey@craftpip.com
 * 
 * Copyright 2013-2014 bootstrap-select Licensed under MIT
 * (https://github.com/craftpip/bootstrap-fullscreen-select/blob/master/LICENSE)
 */
...
(function(b) {
	b.fn.mobileSelect = function(c) {
		...
		d.each(function(g, f) {
			...
			if (e[0].tagName !== "SELECT") {
				console.warn("Sorry, cannot initialized a " + e[0].tagName
						+ " element");
				return true
			}
	...
	a.prototype = {
		...
		_buildHTML : function() {
			if (this.$e.attr("data-triggerOn") === undefined) {
				...
				this.$e
						.before('<button type="button" class="btn '
								+ this.style
			...
			this.$c = b('<div class="mobileSelect-container"></div>').addClass(
					this.theme).appendTo("body");
			this.$c.html(b.fn.mobileSelect.defaults.template);
			this.$c.children("div").css(
					{
						"-webkit-transition" : "all " + this.animationSpeed
								/ 1000 + "s",
						transition : "all " + this.animationSpeed / 1000 + "s"
					}).css(this.padding).addClass("anim-" + this.animation);
			this.$c.find(".mobileSelect-title").html(this.title).end().find(
					".mobileSelect-savebtn").html(this.buttonSave).end().find(
					".mobileSelect-clearbtn").html(this.buttonClear).end()
					.find(".mobileSelect-cancelbtn").html(this.buttonCancel)
					.end();
		...
		_appendOptionsList : function() {
			...
			b.each(this.options, function(g, f) {
				if (f.group && f.group !== d) {
					...
					c.$listcontainer.append('<span class="mobileSelect-group" '
							+ e + ">" + f.group + "</span>");
					d = f.group
				}
				...
				c.$listcontainer
						.append('<a href="#" class="mobileSelect-control" ' + e
								+ ' data-value="' + f.value + '">' + f.text
								+ "</a>")
			});
			this.sync();
		...
		_bindEvents : function() {
			...
			this.$c.find(".mobileSelect-control").on(
					"click",
					function(f) {
						f.preventDefault();
						var d = b(this);
						...
						if (c.isMultiple) {
							...
							d.siblings().removeClass("selected").end()
									.addClass("selected")
						}
					})
		},
		...
		sync : function() {
			...
			for ( var c in d) {
				this.$c.find('a[data-value="' + d[c] + '"]').addClass(
						"selected")
			}
		},
		syncR : function() {
		...
		show : function() {
			...
			var c = this;
			setTimeout(function() {
				c.$c.children("div").removeClass(
						b.mobileSelect.animations.join(" "))
			}, 10);
	...
	b.mobileSelect = {
		elements : {},
		animations : [ "anim-top", "anim-bottom", "anim-left", "anim-right",
				"anim-opacity", "anim-scale", "anim-zoom", "anim-none" ]
	};
