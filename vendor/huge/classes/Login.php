...
class Login
{
    ...
    /**
     *
     * @var string $user_role_id The user's role id
     */
    public $user_role_id = "";
    ...
    /**
     *
     * @var boolean define se o usuario logado eh um fornecedor ou nao
     */
    private $is_fornecedor = 0;
    ...
    // getters e setters -> route Dimension
    ...
    public function setUserRememberMe($user_rememberme)
    {
        $this->user_rememberme = $user_rememberme;
    }
    ...
    public function setIsFornecedor($id_fornecedor)
    {
        $this->is_fornecedor = $id_fornecedor ? 1 : 0;
    }
    ...
    public function __construct()
    {
        // create/read session
        sec_session_start();
        // Dimension -> simply starts a session and collect the action from the Class
        // private $action ... set when the class is instatiated
     ...
     */
    private function getUserData($user_name, $user_email = NULL, $user_role_id, $user_id)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $query_user = $this->db_connection->prepare('' . 'SELECT u.*, ue.*, rb.RoleId ' . 'FROM users u, users_empresa ue, rbac_userroles rb ' . 'WHERE ' . 'u.user_name = :userName AND ' . 'u.user_id = ue.id_user AND ' . 'ue.id_empresa = :idEmpresa AND ' . 'ue.id_fornecedor = :idFornecedor AND rb.RoleId = :RoleId AND rb.UserId = :userId ' . (isset($user_email) ? 'AND ue.user_email = :userEmail' : NULL));
            $query_user->bindValue(':userName', $user_name, PDO::PARAM_STR);
            $query_user->bindValue(':idEmpresa', $this->id_empresa, PDO::PARAM_INT);
            $query_user->bindValue(':idFornecedor', $this->id_fornecedor, PDO::PARAM_INT);
            $query_user->bindValue(':RoleId', $user_role_id, PDO::PARAM_INT);
            $query_user->bindValue(':userId', $user_id, PDO::PARAM_INT);
            isset($user_email) ? bindValue(':userEmail', $user_email) : NULL;
            $query_user->execute();
    ...
    private function getUserDataUnique($user_name)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $query_user = $this->db_connection->prepare('' . 'SELECT u.* ' . 'FROM users u ' . 'WHERE u.user_name = :userName');
            $query_user->bindValue(':userName', $user_name, PDO::PARAM_STR);
            $query_user->execute();
            // get result row (as an object)
            return $query_user->fetchObject();
    ...
    private function loginWithCookieData()
    {
        if (isset($_COOKIE['rememberme'])) {
            ...
            list ($user_id, $token, $hash, $id_empresa, $id_fornecedor) = explode(':', $_COOKIE['rememberme']);
            // check cookie hash validity
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && ! empty($token)) {
                // cookie looks good, try to select corresponding user
                if ($this->databaseConnection()) {
                    // get real token from database (and all other data)
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email, user_complete_name FROM users WHERE user_id = :user_id
                    ...
                    if (isset($result_row->user_id)) {
                        ...
                        $this->gravaSessao($result_row->user_id, $result_row->user_name, $result_row->user_email, $this->user_is_logged_in, $id_empresa, $id_fornecedor, $result_row->user_complete_name);
    ...
    private function loginWithPostData()
    {
        ...
        if (! filter_var($this->user_name, FILTER_VALIDATE_EMAIL)) {
            // database query, getting all the info of the selected user
            $result_row = $this->getUserData(trim($this->user_name), NULL, $this->user_role_id, $this->user_id);
            // if user has typed a valid email address, we try to identify him with his user_email
        } else if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $query_user = $this->db_connection->prepare('SELECT u.*, ue.*, rb.RoleId FROM users u, users_empresa ue, rbac_userroles rb WHERE ue.user_email = :userEmail AND u.user_id = ue.id_user AND ue.id_empresa = :idEmpresa AND ue.id_fornecedor = :idFornecedor AND rb.RoleId = :RoleId AND rb.UserId = :userId');
            $query_user->bindValue(':userEmail', trim($this->user_name), PDO::PARAM_STR);
            $query_user->bindValue(':idEmpresa', $this->id_empresa, PDO::PARAM_INT);
            $query_user->bindValue(':idFornecedor', $this->id_fornecedor, PDO::PARAM_INT);
            $query_user->bindValue(':RoleId', $this->user_role_id, PDO::PARAM_INT);
            $query_user->bindValue(':userId', $this->user_id, PDO::PARAM_INT);
            $query_user->execute();
            // get result row (as an object)
        ...
        if (! isset($result_row->user_id)) {
        ...
        } else if (! password_verify($this->user_password, $result_row->user_password_hash)) {
            // increment the failed login counter for that user
            $sth = $this->db_connection->prepare('UPDATE users ' . 'SET user_failed_logins = user_failed_logins+1, user_last_failed_login = :user_last_failed_login ' . 'WHERE user_name = :user_name OR user_email = :user_name');
            $sth->execute(array(
            ...
            $this->user_is_logged_in = TRUE;
            $this->gravaSessao($result_row->user_id, $result_row->user_name, $result_row->user_email, $this->user_is_logged_in, $this->id_empresa, $this->id_fornecedor, $result_row->user_complete_name, $result_row->RoleId);
            // write user data into PHP SESSION [a file on your server]
            // declare user id, set the login status to true
            ...
            if (isset($this->user_rememberme)) {
                $this->newRememberMeCookie($this->id_empresa, $this->id_fornecedor);
            } else {
            ...
            if (defined('HASH_COST_FACTOR')) {
                ...
                if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array(
     ...
     */
    private function newRememberMeCookie($id_empresa, $id_fornecedor)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            ...
            $cookie_string_first_part = $_SESSION['user_id'] . ':' . $random_token_string . ':' . $id_empresa . ':' . $id_fornecedor;
    ...
    public function isUserLoggedIn()
    {
        ...
        // powered by Dimension
        return isset($_SESSION['user_logged_in']) ? $_SESSION['user_logged_in'] : 0;
    }
     ...
     */
    public function editUserPassword($user_name, $user_password_old, $user_password_new, $user_password_repeat)
    {
        if (empty($user_name) || empty($user_password_new) || empty($user_password_repeat) || empty($user_password_old)) {
            ...
            $result_row = $this->getUserDataUnique($user_name);
            // if this user exists
            if (isset($result_row->user_password_hash)) {
                ...
                if (password_verify($user_password_old, $result_row->user_password_hash)) {
                    ...
                    $user_password_hash = password_hash($user_password_new, PASSWORD_DEFAULT, array(
                        'cost' => $hash_cost_factor
                    ));
     ...
     */
    public function setPasswordResetDatabaseTokenAndSendMail($user_name, $user_email)
    {
            ...
            if (isset($result_row->user_id)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    if ($query_update->rowCount() == 1) {
                        ...
                        if (PRODUCAO) {
                            $this->sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash);
                        }
                        return true;
                    } else {
    ...
    /*
     * funcao que retorna os emails do usuario via id unico
     */
    public function getEmailFromUniqueId($userName)
    {
        // database query:
        if ($this->databaseConnection()) {
            $query_user = $this->db_connection->prepare('' . 'SELECT ue.user_email, ue.id_user ' . 'FROM users_empresa ue, users u ' . 'WHERE ue.id_user = u.user_id AND ' . 'u.user_name = :userName ' . 'ORDER BY user_email');
    ...
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        $link = EMAIL_PASSWORDRESET_URL . '&u=' . $converter->encode($user_name) . '&v=' . urlencode($user_password_reset_hash);
        $mensagem = EMAIL_PASSWORDRESET_CONTENT . ' ' . $link;
        $verification = sha1($mensagem . date('Ymdhis'));
    ...
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code)
    {
        ...
        if (empty($user_name) || empty($verification_code)) {
            ...
            $this->password_reset_link_is_valid = false;
        } else {
            // database query, getting all the info of the selected user
            $result_row = $this->getUserDataUnique($user_name);
     ...
     *            The email address
     * @param string $s
     *            Size in pixels, defaults to 50px [ 1 - 2048 ]
     * @param string $d
     ...
     */
    public function getGravatarImageUrl($email, $ret = false, $s = 50, $d = 'mm', $r = 'g', $atts = array())
    {
        $url = 'http://www.gravatar.com/avatar/';
        $url .= md5(strtolower(trim($email)));
        $url .= '?d=http%3A%2F%2Fpfdimension.com.br/pf/img/user.jpg';
        // $url .= "?s=$s&d=$d&r=$r&f=y";
        // the image url (on gravatarr servers), will return in something like
        // http://www.gravatar.com/avatar/205e460b479e2e5b48aec07710c08d50?s=80&d=mm&r=g
        // note: the url does NOT have something like .jpg
        $this->user_gravatar_image_url = $url;
        $_SESSION['user_gravatar_image_tag'] = $url;
        if ($ret) {
            return $url;
        }
    }

    public function gravaSessao($user_id, $user_name, $user_email, $user_logged_in, $id_empresa, $id_fornecedor, $complete_name, $role_id)
    {
        /*
         * seta a variavel fornecedor para true ou false dependendo do id
         */
        $this->setIsFornecedor($id_fornecedor);
        /*
         * seta as variaveis iniciais da sessao
         */
        ...
        $_SESSION['is_fornecedor'] = $id_fornecedor ? 1 : 0;
        $_SESSION['complete_name'] = $complete_name;
        $_SESSION['id_contagem'] = '';
        $_SESSION['nome_sessao'] = md5('DIM.' . $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT']);
        /*
         * sigla e path para empresa e fornecedor no popover do perfil de usuario
         * classes com setLog() necessitam de $_SESSION['user_email']
         * 03/02/2017 - Jose Claudio - corrigido isso com a verificacao isset
         */
        $empresa = new Empresa();
        $fornecedor = new Fornecedor();
        $cliente = new Cliente();
        $users_empresa = new UsersEmpresa();
        $idCliente = $users_empresa->getIdClienteUsuario($id_empresa, $id_fornecedor, $user_id);
        $siglaEmpresa = $empresa->getSigla($id_empresa);
        $siglaFornecedor = $fornecedor->getSigla($id_fornecedor);
        $tipoFornecedor = $fornecedor->getTipo($id_fornecedor);
        $siglaCliente = $cliente->getSigla($idCliente);
        /*
         * estabelece as principais variaveis da sessao
         */
        $_SESSION['tipo_fornecedor'] = $tipoFornecedor;
        $_SESSION['path_empresa_fornecedor'] = $id_fornecedor ? $siglaEmpresa . '&nbsp;&gt;&gt;&nbsp;' . $siglaFornecedor : $siglaEmpresa . ($idCliente ? '&nbsp;(' . $siglaCliente . ')' : '');
        $_SESSION['id_cliente'] = $id_fornecedor ? getIdClienteFornecedor() : $idCliente;
        /*
         * role do usuario atual
         */
        $usuario = new Usuario();
        $_SESSION['user_role'] = $usuario->getUserRole($user_id, TRUE, $role_id);
        /*
         * instancia a abrangencia das contagens
         */
        $abrangencia = new Abrangencia();
        $_SESSION['abrangencia'] = $abrangencia->listaAbrangencia();
    }
