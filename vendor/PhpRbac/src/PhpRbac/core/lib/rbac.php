...
class RbacManager extends JModel
{
    ...
    /**
     * Dimension
     * retorna todas as permissoes do usuario em um array para a sessao
     ...
     * @return array
     */
    function checkAll($UserID = null, $id_empresa = null, $id_fornecedor = null, $role_id = null)
    {
        if ($UserID === null || $id_empresa === null || $id_fornecedor === null || $role_id === null)
            throw new \RbacUserNotProvidedException("ID User and Id Empresa are required arguments.");

        $sql = "select p.title
            from 
                {$this->tablePrefix()}permissions p,
                {$this->tablePrefix()}rolepermissions rp,
                {$this->tablePrefix()}roles r,
                {$this->tablePrefix()}userroles u
            where
        ...
        $Res = Jf::sql($sql, $UserID, $id_empresa, $id_fornecedor, $role_id);

        return $Res;
    }

    function checkPermissions($UserID = null, $id_empresa = null)
    {
        if ($UserID === null || $id_empresa === null)
            throw new \RbacUserNotProvidedException("\$UserID is a required argument.");
    ...
    /**
     *
     * @return array an array containing all roles
     */
    function getAllRoles()
    {
        $sql = "select r.ID, r.short_name from {$this->tablePrefix()}roles r ORDER BY short_name ASC";
        $stm = DB::prepare($sql);
        $stm->execute();
    ...
    {
        $sql = "select " . "p.ID, p.Description, p.Title " . "from " . "{$this->tablePrefix()}permissions p, " . "{$this->tablePrefix()}roles r, " . "{$this->tablePrefix()}rolepermissions rp " . "where " . "r.ID = rp.RoleID AND " . "p.ID = rp.PermissionID AND " . "r.ID = :id";
        $stm = DB::prepare($sql);
