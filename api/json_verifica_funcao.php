...
if ($login->isUserLoggedIn() && verificaSessao()) {
    if (NULL !== filter_input(INPUT_POST, 'i')) {
        ...
        $cn->setTable($table);
        $fn->setTable($table);
        $aj->setTable('fator_impacto');
        $id = (int) filter_input(INPUT_POST, 'i');
        $linha = $fn->consulta($id);
        $ret = array();
        $valorImpacto = $aj->consultaFatorImpacto($linha['impacto']);
        $selectImpacto = $valorImpacto['id'] . ';' . $valorImpacto['fator'] . ';' . $valorImpacto['sigla'] . ';' . $valorImpacto['tipo']; //valor para a caixa de selecao ID;Impacto;Sigla
